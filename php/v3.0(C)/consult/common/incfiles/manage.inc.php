...
class ui extends page {
  ...
  public static function moduleList()
  {
    ...
    if (!is_null($db))
    {
      ...
      $batchAry = array();
      if ($account -> checkCurrentGenrePopedom('dispose')) array_push($batchAry, 'dispose');
    ...
    else
    {
      ...
      if (count($error) == 0)
      {
        ...
        if (!is_null($db))
        {
          ...
          if ($account -> checkCurrentGenrePopedom('dispose')) $preset[$prefix . 'dispose'] = base::getNum(request::getPost('dispose'), 0);
  ...
  public static function moduleActionBatch()
  {
    ...
    if (base::checkIDAry($ids))
    {
      ...
      if (!is_null($db))
      {
        ...
        else if ($batch == 'dispose' && $account -> checkCurrentGenrePopedom('dispose'))
        {
          if ($db -> fieldSwitch($table, $prefix, 'dispose', $ids)) $status = 1;
        }
      }
      if ($status == 1)
      {
